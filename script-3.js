...
function createSynthPad(ctx, freq, time) {
    ...
    // НЕ запускаем здесь - запустим позже
...
class Visualizer {
    ...
    drawBars() {
        ...
        for (let i = 0; i < barCount; i++) {
            ...
            const gradient = this.getGradient(x, y, x, this.height, hue);
    ...
    getGradient(x1, y1, x2, y2, hue) {
        const key = `${Math.round(x1)},${Math.round(y1)},${Math.round(x2)},${Math.round(y2)},${Math.round(hue)}`;
